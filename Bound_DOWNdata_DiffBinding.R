...
#
# Program name      : Bound_DOWNdata_DiffBinding 
#
# Author            : Susana Paço
#
...
#### -- The input is the dataset extracted from the TOBIAS framework for down regulated genes
# just replace it in the code below


#extract data and subset for treg_bound==1
data <- read.table("TobiasOutputTable_DOWNregulatedGenes.txt", sep = "\t", header = TRUE )
...
HMr<-ha2+Heatmap(data_wideHeatmap,
                 name = "Bound_DOWNdata_DiffBinding_RowScaling",
                 col = mycols,
...
svg("Bound_DOWNdata_DiffBinding_RowScaling.svg",h=20, w=30)
...
pdf("Bound_DOWNdata_DiffBinding_RowScaling.pdf",h=35, w=35)
...
png("Bound_DOWNdata_DiffBinding_RowScaling.png",h=3500, w=3500)
...
write.csv(AllClusters, file="Bound_DOWNdata_DiffBinding_RowScaling_AllClusters.csv")

#____gene clusters data

ClusterGenes<-returnlistclusters[[2]]

write.csv(ClusterGenes, file="Bound_DOWNdata_DiffBinding_RowScaling_ClustersGenes.csv")
...
write.csv(ClustersTFBS, file="Bound_DOWNdata_DiffBinding_RowScaling_ClustersTFSB.csv")
...
write.csv(Genes0s, "Bound_DOWNdata_DiffBinding_RowScaling_Genes0s.csv")
