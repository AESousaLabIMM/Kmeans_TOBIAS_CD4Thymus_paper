...
#
# Summary           : Extract Info for Complex Heatmap Matrix and Sidebars
#
#
# Revision History  :
#
...
# ----- Function to Extract Heatmap Data - DiffBinding
GetMatrixData_DiffBinding<-function(data){
  #necessary packages
  library (plyr)
  library(readr)
  library(tidyr)
  ...
  returnlist<-list(data=data, data_wide=data_wide)
...
# ROW DATA
GetRowData_Expression<-function(data){
  ...
  Noco <- read.table("Data/ExpressionData/TregvsTconv_Thy_DEGnoco.txt", sep = "\t", header = TRUE )
  
  Noco<-data.frame(Noco$ensembl_gene_id, Noco$logFC)
  names(Noco)<-c("ensembl_gene_id", "logFC")
  
  RowBar<-merge(Noco, RowBar, by= "ensembl_gene_id")
  
  
  
  
  
  RowBar$type<- ifelse(RowBar$logFC>0, "UpGene", "DownGene")
  RowBar$color<- ifelse(RowBar$logFC>0, "red", "blue")
...
#COLUMN DATA
GetColData_Expression<-function(data){
...
#Matrix and Bar _ DiffBinding

MatrixAndBar_DiffBinding<-function(data){
  MatrixData<-GetMatrixData_DiffBinding(data)
  
  
  data<-MatrixData[[1]]
  data_wide<-MatrixData[[2]]
  
  RowBar<-GetRowData_Expression(data)
  ColData<-GetColData_Expression(data)
